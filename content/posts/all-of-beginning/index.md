...
年代过于久远已经不记得当时用的哪个了，不过现在应该也用不了，没什么参考价值。当时受限于知识储备不太清楚原理，推测是基于 VLAN（虽然平台里是叫虚拟局域网但似乎这个名字指另外特定的东西，所以严格来说是基于 vpn 的 tun 模式）和局域网发现（也就是前面提到的第一种连接目标）。专用服务器也能用，不过受限于 PC 性能的问题最终还是直接使用内置的对局域网开放功能。
